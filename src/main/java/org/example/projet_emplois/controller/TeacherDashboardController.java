...
import org.example.projet_emplois.dao.TeacherDAO;
...
public class TeacherDashboardController {
    ...
    public void setTeacher(Teacher teacher) {
        ...
        setupDashboard(); // Move setup here to ensure it's triggered after injection
    }

    @FXML
    public void initialize() {
        // Removed logic here; everything is initialized in setTeacher()
    }

    public void setupDashboard() {
        welcomeLabel.setText("Bienvenue, " + teacher.getName());
        drawGridSkeleton();
        populateWeeks();
    ...
    private void loadScheduleForSelectedWeek() {
        if (teacher == null || weekComboBox.getValue() == null) return;

        LocalDate weekStart = weekComboBox.getValue();
        List<Schedule> entries = TeacherDAO.getScheduleForWeek(teacher.getId(), weekStart);
